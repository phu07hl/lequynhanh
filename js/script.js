...
let simSpeed = 1;
// Factors to scale automatic sequence delay
// After the very first sequence we schedule one fast follow-up, then revert to a slower cadence
const FIRST_POST_FAST_FACTOR = 0.35; // very quick follow-up after first sequence
const NORMAL_AUTO_LAUNCH_FACTOR = 1.2; // slower, less chaotic regular cadence (larger => longer gap)
let _postFirstFastUsed = false;

// Sequence speed boost: after the first several sequences we temporarily speed simulation
const SEQUENCE_SIM_SPEED_FACTOR = 1.45; // multiply simSpeed during boosted period
const SEQUENCE_SPEED_DURATION = 2400; // ms duration of simulated speed boost
let simSpeedBoostRemaining = 0; // ms remaining for boost
...
// Heart-shaped firework - parametric heart curve
const heartShell = (size = 1) => {
  return {
    shellSize: size,
    heart: true,
    color: COLOR.Red,
    secondColor: Math.random() < 0.5 ? COLOR.White : COLOR.Gold,
    spreadSize: 280 + size * 90,
    starLife: 1000 + size * 200,
    starDensity: 0.4,
    starCount: 45 + size * 15,
    glitter: "light",
    glitterColor: COLOR.White,
  };
};
...
const vietnamFlagShell = (size = 1) => {
  return {
    ...
    starLife: 900 + size * 200,
    starDensity: 1.2,
    starCount: 36 + size * 12,
    glitter: "medium",
    glitterColor: COLOR.Gold,
  };
};

// Dragon - serpentine/wavy pattern with gold and red
const dragonShell = (size = 1) => {
  return {
    shellSize: size,
    dragon: true,
    color: Math.random() < 0.5 ? COLOR.Red : COLOR.Gold,
    secondColor: COLOR.Gold,
    spreadSize: 320 + size * 100,
    starLife: 1100 + size * 250,
    starDensity: 0.5,
    starCount: 55 + size * 20,
    glitter: "heavy",
    glitterColor: COLOR.Gold,
  };
};
...
function randomShellName() {
  const specialShapes = ["Heart", "Dragon", "Vietnam Flag"];
  if (Math.random() < 0.25) {
    return specialShapes[(Math.random() * specialShapes.length) | 0];
  }
...
// Lấy (angle, radius) từ 1 chữ cái để burst thành hình chữ đó - dày, rõ
function getLetterBurstData(letter, maxPoints = 55) {
  const canvas = document.createElement("canvas");
  const ctx = canvas.getContext("2d");
  const size = 200;
  canvas.width = size;
  canvas.height = size;
  ctx.font = "900 150px 'Be Vietnam Pro', Arial Black, sans-serif";
  ...
  for (let py = 0; py < size; py += step) {
    for (let px = 0; px < size; px += step) {
      ...
      if (imgData.data[i + 3] > 128) {
        const dx = (px - cx) / cx;
        const dy = (py - cy) / cy;
        const radius = Math.sqrt(dx * dx + dy * dy) || 0.1;
...
// Màu sáng, dễ nhận trên nền tối
const LETTER_COLORS = [COLOR.Red, COLOR.Gold];

// 10 quả pháo đầu: nổ → hiện chữ "Lê Quỳnh Anh" rõ ràng, liền mạch, góc cạnh sắc
function launchLeQuynhAnhFireworks() {
  const letters = ["L", "ê", "Q", "u", "ỳ", "n", "h", "A", "n", "h"];
  const baseX = 0.12;
  // Base step between adjacent letters (smaller = letters closer together)
  const stepX = 0.035;
  // Extra gap inserted between words (normalized units)
  const extraGap = 0.06; // reduced so words are closer together
  // Determine burst height so explosions appear below the title with a small gap.
  // We'll compute a normalized launchHeight value (0..1) based on the title's CSS position.
  const titleEl = document.querySelector('.newyear-title') || document.querySelector('#name-display') || document.querySelector('.newyear-overlay');
  const canvasRect = mainStage.canvas.getBoundingClientRect();
  // fallback normalized value if DOM measurement fails
  let burstY = 0.70;
  if (titleEl && canvasRect.height > 0) {
    const titleRect = titleEl.getBoundingClientRect();
    const gapPx = 28; // pixels gap between title bottom and explosion row
    const desiredCssY = titleRect.bottom + gapPx;
    // map CSS Y to stage pixels
    const desiredStageY = ((desiredCssY - canvasRect.top) / canvasRect.height) * stageH;
    const minHeightPx = stageH * 0.55; // same calc as in Shell.launch()
    const vpad = 50; // same vpad used in Shell.launch()
    let launchHeightNormalized = (minHeightPx - desiredStageY) / (minHeightPx - vpad);
    launchHeightNormalized = Math.max(0, Math.min(1, launchHeightNormalized));
    burstY = launchHeightNormalized;
  }

  // Compute normalized horizontal positions, inserting extra gaps between words
  // Word boundaries for "Lê Quỳnh Anh": after index 1 (Lê) and after index 6 (Quỳnh)
  const wordBoundaries = new Set([1, 6]);
  // Precompute total span so we can center the whole name horizontally
  let gapsCount = 0;
  for (let i = 0; i < letters.length; i++) if (wordBoundaries.has(i)) gapsCount++;
  const totalSpan = (letters.length - 1) * stepX + gapsCount * extraGap;
  const start = Math.max(0.02, 0.5 - totalSpan / 2); // keep a small left margin
  const positions = [];
  let cursor = start;
  for (let i = 0; i < letters.length; i++) {
    ...
    // advance cursor
    cursor += stepX;
    if (wordBoundaries.has(i)) cursor += extraGap;
    const shell = new Shell({
      shellSize: 1.3,
      spreadSize: 420,
      // shorter life for faster-looking bursts
      starLife: 3600,
      starLifeVariation: 0.08,
      starDensity: 1.1,
      color: LETTER_COLORS[i % 2],
      glitter: "medium",
      glitterColor: COLOR.Gold,
      letter: letters[i],
      letterData: getLetterBurstData(letters[i], 60),
      letterLaunch: true,
      fastLaunch: true,
    });
    // Launch immediately (simultaneous)
    shell.launch(positions[i], burstY);
  }

  // DOM letters will be created at burst time inside Shell.burst()
}
...
function init() {
  ...
  // "Chúc mừng năm mới 2026" luôn hiển thị - không biến mất
  ...
  // Begin simulation: keep paused until midnight; show cooldown timer
  togglePause(true);
  startCooldownUntilMidnight();
...
// Cooldown until local midnight (00:00). Shows `#cooldown` and unpauses when time reaches midnight.
...
function startCooldownUntilMidnight() {
  const el = document.getElementById("cooldown");
  const btn = document.getElementById("skip-cooldown");
  if (!el) {
    // no UI to show, just start immediately
    togglePause(false);
    return;
  }
  if (_cooldownInterval) clearInterval(_cooldownInterval);
  // Attach skip handler once
  if (btn && !btn._skipAttached) {
  ...
  function tick() {
    ...
    el.textContent = `Bắt đầu sau: ${formatDuration(ms)}`;
    // ensure button visible while countdown active
...
function seqRandomShell() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const offset = (i - count/2) * 0.06 + (Math.random() - 0.5) * 0.08;
    shell.launch(size.x + offset, size.height);
    if (shell.starLife > maxLife) maxLife = shell.starLife;
    if (shell.fallingLeaves) maxLife = Math.max(maxLife, 4600);
  }
  return 400 + Math.random() * 300 + maxLife * 0.35;
}
...
function seqTwoRandom() {
  const positions = [0.12, 0.25, 0.38, 0.5, 0.62, 0.75, 0.88];
  let maxLife = 0;
  positions.forEach((baseX, i) => {
    ...
    setTimeout(() => {
      shell.launch(baseX + offset, size.height);
    }, i * 50);
    if (shell.starLife > maxLife) maxLife = shell.starLife;
    if (shell.fallingLeaves) maxLife = Math.max(maxLife, 4600);
  });
  return 350 + Math.random() * 250 + maxLife * 0.3;
}
...
function seqTriple() {
  ...
  const positions = [[0.5, 0.7], [0.15, 0.12], [0.85, 0.12], [0.3, 0.45], [0.7, 0.45], [0.2, 0.65], [0.8, 0.65]];

  positions.forEach(([x, y], i) => {
    const size = i === 0 ? baseSize : smallSize;
    setTimeout(() => {
      const shell = new Shell(shellType(size));
      const offset = (Math.random() - 0.5) * 0.08;
      shell.launch(x + offset, y);
    }, i * 80);
  });
  return 2200;
}
...
function seqPyramid() {
  const barrageCountHalf = IS_DESKTOP ? 15 : 9;
  ...
  return 2200 + barrageCountHalf * 180;
}

function seqSmallBarrage() {
  seqSmallBarrage.lastCalled = Date.now();
  const barrageCount = IS_DESKTOP ? 23 : 13;
...
function startSequence() {
  if (isFirstSeq) {
    ...
    if (IS_HEADER) {
      ...
      // On first run show the 10-shell name volley (simultaneous)
      launchLeQuynhAnhFireworks();
      return 8000;
    }
  }
...
function updateGlobals(timeStep, lag) {
  ...
  if (store.state.config.autoLaunch) {
    ...
    if (autoLaunchTime <= 0) {
      const nextDelay = startSequence();
      if (!_postFirstFastUsed) {
        // quick follow-up once after first sequence
        autoLaunchTime = nextDelay * FIRST_POST_FAST_FACTOR;
        _postFirstFastUsed = true;
      } else {
        // normal cadence after first, but speed up simulation briefly during the sequence
        autoLaunchTime = nextDelay * NORMAL_AUTO_LAUNCH_FACTOR;
        // activate temporary sim speed boost for the next active period
        simSpeedBoostRemaining = SEQUENCE_SPEED_DURATION;
      }
    }
  }

  // decrement any active sim-speed boost
  if (simSpeedBoostRemaining > 0) {
    simSpeedBoostRemaining -= timeStep;
    if (simSpeedBoostRemaining < 0) simSpeedBoostRemaining = 0;
  }
}
...
function update(frameTime, lag) {
  ...
  const width = stageW;
  const height = stageH;
  const baseTimeStep = frameTime * simSpeed;
  updateGlobals(baseTimeStep, lag);

  // apply temporary sim-speed multiplier when boost active
  const simMultiplier = simSpeedBoostRemaining > 0 ? SEQUENCE_SIM_SPEED_FACTOR : 1;
  const timeStep = baseTimeStep * simMultiplier;
  const speed = simSpeed * simMultiplier * lag;
...
class Shell {
  ...
  launch(position, launchHeight) {
    ...
    const velocityMult = this.horsetail ? 1.2 : this.letterLaunch ? 0.85 : this.fastLaunch ? 1.5 : 1.15;
    const lifeMult = this.letterLaunch ? 420 : this.fastLaunch ? 200 : this.horsetail ? 100 : 300;
    ...
    const comet = (this.comet = Star.add(
      ...
      typeof this.color === "string" && this.color !== "random"
        ...
        : COLOR.White,
      Math.PI,
      launchVelocity * velocityMult,
      launchVelocity * lifeMult
    ));
  ...
  burst(x, y) {
    ...
    if (typeof this.color === "string") {
      ...
      // Letter shape - mỗi quả nổ thành 1 chữ cái
      if (this.letter && this.letterData && this.letterData.length > 0) {
        this.letterData.forEach(({ angle, radius }) => {
          const speedMult = 0.5 + radius * 0.9 + Math.random() * 0.2;
          starFactory(angle, speedMult);
        });
        // Create a DOM letter at the explosion position so the character appears when bursting
        try {
          const lettersContainer = document.getElementById("letters-container");
          if (lettersContainer) {
            const rect = mainStage.canvas.getBoundingClientRect();
            // x,y are in stage coordinate space (stageW x stageH). Map those to CSS pixels
            const cssX = rect.left + (x / stageW) * rect.width;
            const cssY = rect.top + (y / stageH) * rect.height;
            const el = document.createElement("div");
            el.className = "letter-particle";
            el.textContent = this.letter;
            el.style.left = cssX + "px";
            el.style.top = cssY + "px";
            el.style.color = typeof this.color === 'string' ? this.color : (LETTER_COLORS[0] || COLOR.White);
            lettersContainer.appendChild(el);
            requestAnimationFrame(() => el.classList.add("visible"));
            setTimeout(() => {
              if (el.parentNode) el.parentNode.removeChild(el);
            }, Math.max(2000, this.starLife || 4000));
          }
        } catch (e) {
          // Defensive: don't crash fireworks if DOM positioning fails
          console.warn('Failed to create DOM letter at burst', e);
        }
      }
      // Heart shape - parametric heart curve
      else if (this.heart) {
        const count = this.starCount || 50;
        for (let i = 0; i < count; i++) {
          ...
          const dist = Math.sqrt(hx*hx + hy*hy) * scale;
          const speedMult = 0.6 + dist * 0.8 + Math.random() * 0.3;
          starFactory(angle, speedMult);
        }
        if (this.secondColor) {
          color = this.secondColor;
          for (let i = 0; i < Math.floor(count / 3); i++) {
            const t = (i / count) * PI_2 + 0.5;
            const hx = 16 * Math.pow(Math.sin(t), 3);
            const hy = 13 * Math.cos(t) - 5 * Math.cos(2*t) - 2 * Math.cos(3*t) - Math.cos(4*t);
            starFactory(Math.atan2(hy, hx), 0.5 + Math.random() * 0.4);
          }
        }
      }
      // Vietnam flag - 5-pointed star
      else if (this.vietnamFlag) {
        const count = this.starCount || 40;
        const starPoints = 5;
        for (let i = 0; i < count; i++) {
          const arm = i % starPoints;
          const dist = Math.floor(i / starPoints) / (count / starPoints);
          const baseAngle = (arm / starPoints) * PI_2 - PI_HALF;
          const angle = baseAngle + (Math.random() - 0.5) * 0.4;
          const speedMult = 0.5 + dist * 1.2 + Math.random() * 0.4;
          starFactory(angle, speedMult);
        }
      }
      // Dragon - serpentine wavy pattern
      else if (this.dragon) {
        const count = this.starCount || 55;
        const arms = 5;
        for (let i = 0; i < count; i++) {
          const armAngle = (i % arms) / arms * PI_2 + Math.random() * 0.2;
          const waveOffset = Math.sin(i * 0.5) * 0.3;
          const angle = armAngle + waveOffset;
          const speedMult = 0.7 + Math.random() * 0.8;
          starFactory(angle, speedMult);
        }
      }
